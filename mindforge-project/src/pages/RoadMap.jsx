...
const RoadMap = () => {
  ...
  const phaseStyle = {
    ...
    backgroundImage: 'url("https://gteches.com/wp-content/uploads/2023/05/Full-Stack-3.gif")',// Default background image
  };
  ...
  const openMindmap = (phase) => {
    ...
    if (componentName) { // If a valid component name is found, open the corresponding mindmap
      const url = `/path-to-components/${componentName}`; 
      window.open(url, '_blank');// Update the path to your components
    }
  };

  const phases = [// Array containing information about each phase
    {
  ...
  return (
    <div style={containerStyle}>{/* Container div with custom styles */}
      <h2 style={titleStyle}>Choose Your MindMap Here :</h2>{/* Title */}
      <div>
